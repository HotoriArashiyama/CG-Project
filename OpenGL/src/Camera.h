...
public:
	...
	static void SightMouseFollow(double x, double y);
	...
	void SetPrevXY(double x, double y) { prevX = x, prevY = y; }
...
namespace Camera_h {
	...
	enum CameraStatus { FOLLOWING_MOUSE = 0, FOLLOWING_MOUSE_NO_MOVE, INTERACTING, DRAG_ONLY, DRAG_ONLY_NO_MOVE };
	static int MAIN_CAMERA_STATUS = FOLLOWING_MOUSE;
}
...
void Camera::SightMouseFollow(int x, int y) {
	double dbx = x, dby = y;
	SightMouseFollow(dbx, dby);
}

void Camera::SightMouseFollow(double x, double y) {
	if (Camera_h::ptrMainCam && Camera_h::MAIN_CAMERA_STATUS != Camera_h::DRAG_ONLY && Camera_h::MAIN_CAMERA_STATUS != Camera_h::DRAG_ONLY_NO_MOVE) {
	...
	if (Camera_h::MAIN_CAMERA_STATUS == Camera_h::DRAG_ONLY || Camera_h::MAIN_CAMERA_STATUS == Camera_h::DRAG_ONLY_NO_MOVE) {
		Camera_h::ptrMainCam->prevX = x, Camera_h::ptrMainCam->prevY = y;
	}
}
